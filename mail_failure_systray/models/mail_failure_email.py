...
class MailFailureEmail(models.Model):
    ...
    _mail_unique = models.Constraint('UNIQUE(mail_id)', 'An email can only be tracked once.')
    _owner_company_state_date = models.Index('(owner_id, company_id, state, created_date DESC)')
    ...
    def _search(self, domain, offset=0, limit=None, order=None, *, bypass_access=False, **kwargs):
        ...
        candidates = super()._search(domain, order=order, **kwargs)
        ids = [row[0] for row in self.env.execute_query(candidates.select())]
        allowed = self.browse(ids)._document_allowed_ids()
        return super()._search(Domain(domain) & Domain('id', 'in', allowed), offset, limit, order, **kwargs)
